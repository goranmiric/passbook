 ...
 */
class PassbookViewController extends EntityViewController {
   ...
   * {@inheritdoc}
   */
  public function view(EntityInterface $passbook, $view_mode = 'full', $langcode = NULL) {
    $build = parent::view($passbook, $view_mode, $langcode);

    foreach ($passbook->uriRelationships() as $rel) {
      $url = $passbook->toUrl($rel);
      ...
      if ($this->currentUser->isAuthenticated() || $url->access($this->currentUser)) {
        // Set entity path as the canonical URL to prevent duplicate content.
   ...
   *
   * @param \Drupal\Core\Entity\EntityInterface $passbook
   *   The current entity.
   *
   * @return string
   *   The page title.
   */
  public function title(EntityInterface $passbook) {
    return $this->entityManager->getTranslationFromContext($passbook)->label();
  }
